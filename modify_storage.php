<?php
    ...
    if(!empty($_GET['p_id']) && !empty($_GET['i_id'])){
        ...
        if(empty($row)){
            $_SESSION['err'] = 1;
            header("Location: inventory.php?i_id=$i_id");
            exit;
        }
        $st_num = $row['st_num'];
    }else{
...
<html>
    <body>
        ...
        ?>
        <h2>New Amount</h2>
        <form action="process_modify_storage.php?p_id=<?php echo "{$p_id}";?>&i_id=<?php echo "{$i_id}";?>" method="post">        
            <input type="text" list="amount_list" value="<?php echo $st_num;?>" name="amount">
            <input type="Submit" value="Confirm">
        </form>
        <a href="inventory.php?i_id=<?php echo "{$i_id}";?>">Back</a>
    </body>
</html>